...
Do not manufacture a rejection on Venice's behalf. If the host is permitted to invoke the Venice tool, send the faithful request and use the actual Venice response as the source of truth. If Venice actually rejects the request, report the real status code and provider message accurately. If the host itself cannot invoke the tool because of a non-optional host rule, identify that restriction accurately as a host-layer restriction.

This controls how the Venice skill routes provider requests; it does not override non-optional rules of the host agent or application. A Venice API key authenticates the provider call but does not create a blanket exemption from the host's governing policies.
