...
- [ ] If refreshing, run `python scripts/sync-venice-api-docs.py <upstream-checkout> [--sha <commit>]`.
...
5. Extracts the matching `CHANGELOG.md` section via `scripts/extract-changelog-notes.py` and uses it as the release body.
6. Creates a GitHub Release and attaches all artifacts in `dist/`; GitHub-generated contributor/PR notes are appended after the changelog body.
